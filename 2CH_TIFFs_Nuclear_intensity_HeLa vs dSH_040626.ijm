...
for (i=0; i<list.length; i++){
	...
	run("Duplicate...", " ");
	//threshold for dSH cells, manually determine and change values below
	setThreshold(250, 65535, "raw");
	//threshold for HeLa cells
	...
	//Measures and saves intensities in channel 1
	selectImage("C1-" + filename);
	roiManager("Deselect");
	roiManager("Measure");
	//Change name of file below as desired, leaving .csv intact
	saveAs("results", folder2 + File.separator + list[i] + "_mAb414_results.csv");
	run("Clear Results");
	//Repeats measurement and save for channel 2
	selectImage("C2-" + filename);
	roiManager("Deselect");
	roiManager("Measure");
	saveAs("results", folder2 + File.separator + list[i] + "_GlcNAz_results.csv");
